...
- Profile polish (round from new-member feedback): **modals are bottom-sheets** on phones (slide up, internal scroll, fixed close — content never cut off), **photo uploads have a drag + zoom crop step**, **"About & details" editor** in-profile (bio, profession, hometown, church, diaspora), **avatar photos follow members** into chat messages, forum threads/replies and DMs (initials fallback on missing/broken images), **phone numbers are stored but never displayed anywhere public**
...
- Home page **"Meet the Circle" strip**: slow rolling marquee of member avatars (visible pre-login) with online/offline dots — **online members first** (most recently active), remaining slots filled with **latest registrants**, hard cap 40; tap shows privacy-light card (region/diaspora, denomination, status) + Join CTA for visitors
