...
1. **DESIGN & TEXT POLISH** — user's stated next want: "polish the design and text etc" now that everything works. Premium $100k look, Ghana flag colors, unique branding. **IN PROGRESS (step by step):**
   - Step 1 ✅ above-the-fold copy (announcement, hero badge/subtitle tightened; user also pasted into live Admin — stored settings win over code defaults, no conflict)
   - Step 2 ✅ landing rhythm & consistency (uniform "All events"/section CTA outline buttons; showcase & carousel bands `py-16 sm:py-24`; Stories section de-merged from soft Live-Rooms band; removed public "Links managed in Admin…" note)
   - Step 3 ✅ shared `EmptyState` component in `ui.tsx` (icon tile + display title + sub + optional action) applied to chat, forum, blog, events, notifications & DM empties with friendlier copy; category-aware wording in blog empty state
   - Step 4 ✅ forms & auth microcopy — REMOVED public demo-credentials hint from login (security: seeded admin@adomcircle.org / Adom@2026 existed on live DB; anyone could sign in as admin → after deploy, reset the admin password via Forgot password + mailbox code, and delete the demo accounts you don't need in Admin → Members); added autocomplete attrs (name/email/tel/current-password/new-password), password helper line, diaspora-friendly note on region picker, tightened Values card & button copy
   - Step 5 ✅ dark-mode & contrast — added class-based `@custom-variant dark` (dark: variants now follow the .dark class); gold chips now keep ink text (cream-on-gold was unreadable in dark); dark buttons/toasts/ink chips get subtle borders in dark mode so they don't vanish into the page; socials dark-tone adapts to dark surfaces; fixed invisible ink stars in the flag marquee (now cream/35)
   - Step 6 ✅ settings wordsheet written to `output/adom-circle-copy-sheet.md` (curated copy for Admin → Site content + honest-stats reminder)
   - Step 7 ✅ RICH TEXT everywhere long-form text is written: `marked` + `DOMPurify` (sanitised) via `src/client/lib/markdown.tsx` (`RichText` renderer + `plainText` preview stripper) and a toolbar `MdEditor` component (`src/client/components/md-editor.tsx` — bold/italic/heading/list/quote/link around the selection). Applied to: blog reading + Admin post editor, forum thread composer + thread/reply rendering (replies render rich; Enter-to-send box hints formatting), project detail + submit editor, home/feed previews strip markdown cleanly. Stored text stays plain markdown — safe, backward-compatible with old plain posts (single newlines render as breaks).
